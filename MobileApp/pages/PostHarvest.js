...
const PostHarvest = () => {
  ...
  const renderPlanItem = ({ item }) => (
    <GlassmorphicCard style={styles.planCard} blurAmount={6}>
  ...
  const renderWeatherItem = ({ item, index }) => (
    <GlassmorphicCard style={styles.weatherCard} blurAmount={6}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer}>
          ...
          >
            {loading ? (
              ...
              <Text style={styles.buttonText}>{t('postHarvest.getPlan')}</Text>
            )}
          </AnimatedButton>
          {error && (
