    ...
    return (
        <View style={{ flex: 1, backgroundColor: 'transparent' }}>
            ...
            >
                ...
                <GlassmorphicCard style={styles.filterCard} blurAmount={8}>
                ...
                {error && (
                    <View style={styles.input}>
                        ...
                        <TouchableOpacity style={styles.retryButton}
                            activeOpacity={0.8}
                        >
                            <Text style={{ fontFamily: theme.font.bold, color: theme.white }}>{t('marketPrices.errors.retry')}</Text>
                        </TouchableOpacity>
                    </View>
                )}

                {!loading && !error && marketPrices.length > 0 && (
                    <GlassmorphicCard style={styles.resultCard} blurAmount={8}>
                        ...
                        <View style={styles.row}>
                            <View style={{ flex: 1, marginRight: 4 }}>
                                ...
                                <Text style={[styles.value, { fontSize: theme.fs2, fontFamily: theme.font.bold, color: theme.primary }]}>{priceInfo.currentPrice}</Text>
                            </View>
                            <View style={{ flex: 1, marginLeft: 4, flexDirection: 'row', alignItems: 'center' }}>
                                <Icon
                                    name={priceInfo.isUp ? 'arrow-up' : 'arrow-down'}
                                    size={20}
                                    color={priceInfo.isUp ? theme.secondary : theme.danger}
                                />
                                <Text style={[styles.value, { color: priceInfo.isUp ? theme.secondary : theme.danger }]}>{priceInfo.change}</Text>
                            </View>
                        </View>
                ...
                {!loading && !error && marketPrices.length > 0 && totalRecords > limit && (
                    <View style={styles.row}>
                        <TouchableOpacity
                            ...
                            disabled={offset === 0}
                        >
                            <Icon name="chevron-left" size={24} color={offset === 0 ? theme.text3 : theme.white} />
                            <Text style={[styles.value, { color: offset === 0 ? theme.text3 : theme.white }]}>{t('marketPrices.actions.previous')}</Text>
                        </TouchableOpacity>

                        <Text style={styles.paginationInfo}>
                            {offset + 1}-{Math.min(offset + limit, totalRecords)} of {totalRecords}
                        </Text>

                        <TouchableOpacity
                            style={[
                                styles.paginationButton,
                                (offset + limit >= totalRecords) && styles.paginationButtonDisabled
                            ]}
                            onPress={handleNext}
                            disabled={offset + limit >= totalRecords}
                        >
                            <Text style={[styles.value, { color: (offset + limit >= totalRecords) ? theme.text3 : theme.white }]}>{t('marketPrices.actions.next')}</Text>
                            <Icon name="chevron-right" size={24} color={(offset + limit >= totalRecords) ? theme.text3 : theme.white} />
                        </TouchableOpacity>
                    </View>
                )}
...
const styles = StyleSheet.create({
    ...
    paginationButtonDisabled: {
        backgroundColor: theme.disabledBackground || '#ccc',
    },
    paginationButton: {
        flexDirection: 'row',
        alignItems: 'center',
